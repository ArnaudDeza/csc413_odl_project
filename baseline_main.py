...
def main(arg, idx=0):
    ...
    plt.savefig("/u/c/dezaarna/Documents/CSC413_PROJECT/ODL/src/baseline/acc.png")
    plt.close()
    plt.plot(list(range(len(cumAverageLoss))),cumAverageLoss)
    plt.savefig("/u/c/dezaarna/Documents/CSC413_PROJECT/ODL/src/baseline/loss.png")
    
    filename = (config['log'] + '_' + str(idx) + '.acc')
    #np.savetxt(filename, cumAverageLoss, delimiter=',')
    
    return cumAverageAcc,cumAverageLoss
